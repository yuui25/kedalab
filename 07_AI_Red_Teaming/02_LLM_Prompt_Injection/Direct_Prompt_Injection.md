...
- 実行環境: 攻撃者マシン（Kali または任意の HTTP クライアントが使える環境）
- 必要なツール: ブラウザ / curl / Burp Suite（Kali標準搭載）
- コールバック受信が必要な場合は `python3 -m http.server` または `nc -lvnp` を事前起動する
- LLM アプリへのポートフォワードが必要な場合: SSH の `-L`（ローカル）と `-R`（リモート）フラグを使用する

```bash
# [Kali] SSH ポートフォワード例（LLM アプリが内部ポートで動作する場合）
# ローカルの 5000 番を対象ホストの 80 番に転送
# リモートの 8000 番をこちらの 8000 番に転送（コールバック受信用）
