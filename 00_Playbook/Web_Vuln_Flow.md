...
- **報告形式** — 脆弱性の発見だけか、PoC（概念実証）まで求められるか
...
**低権限ユーザーで高権限エンドポイントにアクセス可能か確認する。**

1. 低権限アカウント（一般ユーザー）でログインし、高権限エンドポイントのURLを直接叩く
2. 別ユーザーのオブジェクトID（ファイルID・注文IDなど）に連番を変えてアクセスできるか確認する
3. APIエンドポイントに対して `is_admin=1` `role=admin` 等のパラメータ改ざんを試す
