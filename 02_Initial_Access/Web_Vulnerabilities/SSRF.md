...
| リクエストは投げられるがレスポンスが戻らない（ステータス固定・本文なし） | Blind SSRF | 自分の公開 HTTP サーバーへのコールバック・DNS ルックアップで到達を確認 |
...
## 関連技術
- 内部サービスへのアクセスが確認できた → 対象サービスの脆弱性を調査（`../../01_Reconnaissance/Network_Scanning.md` で内部ポートの意味を確認）
- クラウドメタデータから一時クレデンシャル取得 → `../Credential_Discovery.md`
- プロトコル切替で Redis / 内部 SMTP 等に到達 → `../Protocol_Exploitation.md`
- パストラバーサルと併発しやすい（同じ「入力URL/パス」系の脆弱性） → `Path_Traversal.md`
